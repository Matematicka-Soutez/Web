  ...
  describe.only('Team registration: POST /api/competitions/current/registration/:schoolToken', function teamRegistration() {
    ...
    it('FAIL 409 Conflict - should allow only 1 team per school during 1st round', async function getTeams() {
      ...
      await request(this.server)
        ...
        .expect(409, {
          type: 'CONFLICT',
          message: 'V tuto chvíli nemůžete registrovat více než jeden tým z jedné školy.',
        })
    })
    it('FAIL 409 Conflict - should allow only 2 teams per school during 2nd round', async function getTeams() {
      const school = await createSchool()
      await db.Competition.update({
        date: moment().add(7, 'days').toISOString(),
        registrationRound1: moment().subtract(7, 'days').toISOString(),
        registrationRound2: moment().subtract(1, 'days').toISOString(),
        registrationRound3: moment().add(7, 'days').toISOString(),
        registrationEnd: moment().add(9, 'days').toISOString(),
      }, { where: { id: 3 } })
      await request(this.server)
      ...
      await request(this.server)
        ...
        .expect(409, { 
          type: 'CONFLICT',
          message: 'V tuto chvíli nemůžete registrovat více než dva týmy z jedné školy.' })
    })
    it('FAIL 409 Conflict - should allow only 3 teams per school during 3rd round', async function getTeams() {
      const school = await createSchool()
      await db.Competition.update({
        date: moment().add(7, 'days').toISOString(),
        registrationRound1: moment().subtract(9, 'days').toISOString(),
        registrationRound2: moment().subtract(7, 'days').toISOString(),
        registrationRound3: moment().subtract(1, 'days').toISOString(),
        registrationEnd: moment().add(9, 'days').toISOString(),
      }, { where: { id: 3 } })
      await request(this.server)
      ...
      await request(this.server)
          ...
          .expect(409, { 
            type: 'CONFLICT',
            message: 'V tuto chvíli nemůžete registrovat více než tři týmy z jedné školy.' })  
    })
  })
})
